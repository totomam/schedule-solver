...
- Reilly Weakley: hard-capped at 3 shifts/week (same pattern as Jacob Cothern's 2-shift cap) — his `FT_NONLEADER` floor is overridden to 24h (not the standard 33h) to match what 3 shifts can actually deliver, and skips the usual +1 buffer since 24h is a real hard ceiling, not a headroom estimate.
