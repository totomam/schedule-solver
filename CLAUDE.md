...
- Zac Duffy: no hard hour cap — targets 30h with a penalty for missing it (`_HPEN_FT`/510 tier via `FT_nonleader`), then falls under the generic 40h ceiling like everyone else. (Gracelyn Dailey's old 30h override was removed — she now uses the standard `strong_PT` 18h target like the rest of that group.)
- Reilly Weakley: hard-capped at 3 shifts/week (same pattern as Jacob Cothern's old standalone 2-shift cap, before Jacob moved into `weak5` — see "People groups" below) — his `FT_NONLEADER` floor is overridden to 24h (not the standard 30h) to match what 3 shifts can actually deliver, using the standard generic-loop floor+1 buffer like everyone else (no special case needed in `solver2.py`). Note the buffer is a no-op for him specifically: with floor == his real ceiling (no headroom above 24h to push into), `expr+slack>=24` and `expr+slack>=25` yield the identical optimal `expr` for every feasible value — the +1 trick only helps when true-max > floor. A 50-run stress batch showed the solver occasionally landing him at 23h even when 24h was reachable (ordinary MIP/gapRel imprecision on a floor quantized in whole 8h-shift increments, same class as Trinity Stringer's frequent 0.2h misses, just landing in a bigger unit). That's not a solver defect worth chasing — it's absorbed via `grid_tolerance()` in `backbone.py` (shared by `solver2.py`'s live audit and `test_protocol.py`'s classifier): anyone with a below-default `SHIFT_CAP` gets a wider grid-tolerance allowance (`SHIFT_CAP_GRID_TOLERANCE_H`=1.5h) than the generic `GRID_TOLERANCE_H`=0.5h, since their achievable hours move in whole-shift jumps, not smooth fractions. A shortfall within tolerance isn't printed as a `HoursUnder` issue at all.
