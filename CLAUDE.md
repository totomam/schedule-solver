...
- Reilly Weakley: hard-capped at 3 shifts/week (same pattern as Jacob Cothern's 2-shift cap) — his `FT_NONLEADER` floor is overridden to 24h (not the standard 33h) to match what 3 shifts can actually deliver. Uses the standard +1 buffer like every other floor (see "Hours floor buffer" below) — an earlier version of this override skipped the buffer on the theory that a floor+1 target would be "permanently unreachable" past his hard 24h ceiling, but that's exactly backwards: the buffer's job is to counteract gapRel early-stop undershoot, and the audit only ever checks the real floor (`afl`), never floor+1, so skipping it just left the 24h target with no protection — stress testing caught the solver consistently landing at 23h instead.
